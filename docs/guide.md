...
- **Analysis page** (button on every run with data): codings per code, normalized comparison across document groups (with deviation from the cross-group mean), code co-occurrence matrix, documents × codes with coverage, and lemma frequencies over the coded segments per language (raw and normalized, filtered by the workspace's per-language stoplists — set them in Settings to remove transcription artifacts and empty verbs). Dictionary runs add **expression firing counts** (split per language, with the same Code and Group drill-down as the lemmas) and the **top extracts** per code by relevance score (collapsible per-code accordions, filterable by code and group). When the codebook uses **clusters**, three more blocks appear: units per cluster, clusters by group (the same normalized comparison one level up), and a cluster × cluster co-occurrence matrix — usually the readable version of a code matrix too large to look at. Cluster figures are counted on **distinct units, not by summing the code rows**: a unit carrying two codes of the same family counts once, and since that inflation differs from family to family, summing would distort exactly the between-family comparison. Codes with no cluster appear under *(no cluster)* rather than being dropped, and the blocks are absent entirely from workspaces that use no clusters. Every chart downloads as publication-ready PNG or PDF; all underlying data exports as one Excel workbook, with the cluster carried alongside the code in the code-level sheets. The analysis is computed on demand — on a large corpus a progress bar shows the lemma pass — then cached (runs are immutable); use **Recompute** after changing the stoplists.
